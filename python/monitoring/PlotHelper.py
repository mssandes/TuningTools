...
def PileupEffHist( pileupPass, pileup, bins, name ):
  ...
  for p in pileup: htotal.Fill( p )
  for p in pileupPass: hpass.Fill( p )
  #pileup = pileup.astype( np.float64 )
  #pileupPass = pileupPass.astype( np.float64 )
  #wtotal = np.ones( shape=pileup.shape, dtype = np.float64 )
  #wpass = np.ones( shape=pileupPass.shape, dtype = np.float64 )
  #htotal.FillN( len(pileup),  pileup, wtotal  )
  #hpass.FillN( len(pileupPass), pileupPass, wpass )
