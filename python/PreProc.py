...
__all__secondary__ = ["ReshapeToVortex", "ReshapeToVortexOnlyEM", "ReshapeToVortexOnlyHAD", "ReshapeToCells"]
...
class ReshapeToCells(Logger):
  ...
  def _make_quarter_rings( self, rings, scale ):
    ...
    norms = self._retrieveNorm(rings)
    rings = rings/norms
    center = np.where(mask==1)
    cx = center[0][0]; cy=center[1][0]
    mask = np.array([ mask[i][0:cy] for i in range(0,cx)])
    ...
    for r in range(len(rings)):
      ...
      for c in range(len(pos[0])):
        cells[pos[0][c]][pos[1][c]][:] = 10* rings.T[r][:] / float(len(pos[0])/float(scale*scale))  
    return cells
