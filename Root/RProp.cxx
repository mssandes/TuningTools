...
//==============================================================================
RProp::RProp(INeuralNetwork *net, Level msglevel) 
  : Backpropagation(net, msglevel), 
    m_msgLevel(msglevel)
{
  ///Application name is set by default to MsgStream monitoring
  m_appName  = "RProp";
...
void RProp::updateWeights(const unsigned /*numEvents*/)
{
  for (unsigned i=0; i<(nNodes.size()-1); i++)
  {
    for (unsigned j=0; j<nNodes[(i+1)]; j++)
    {
      ...
      if (frozenNode[i][j])
      {
        MSG_DEBUG(m_log, "Skipping updating node " << j 
            << " from hidden layer " << i 
            << ", since it is frozen!");
        for (unsigned k=0; k<nNodes[i]; k++) {
          dw[i][j][k] = 0;
...
void RProp::showInfo() const
{
  ...
  MSG_INFO(m_log, "Maximum allowed learning rate value (deltaMax) = " 
      << deltaMax);
  MSG_INFO(m_log, "Minimum allowed learning rate value (deltaMin) = " 
      << deltaMin);
